...
class MoviesList : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (i in movies.indices) {
            ...
            val resID = resources.getIdentifier(imgjpg, "drawable", packageName)
            ...
            listView.setOnItemClickListener { parent: AdapterView<*>, view: View, position: Int, id: Long ->
                val appInfo = Intent(this, MovieDetail::class.java)

                val p =  listView.getItemIdAtPosition(position).toInt()

                data class Detail (val title:String?, var year:String?, var director:String?,
                                   var image:String?, var Description:String?):
                    Serializable

                val tt = (movies[p][0])
                appInfo.putExtra("title", tt)
                val yr = (movies[p][1])
                appInfo.putExtra("year", yr)
                val dr = (movies[p][2])
                appInfo.putExtra("director", dr)
                val des = (movies[p][4])
                appInfo.putExtra("description", des)

                //parsing image name
                val imgjpg = movies[p][0]
                    .lowercase()
                    .replace("28", "")
                    .trim()
                    .replace(" ", "_")
